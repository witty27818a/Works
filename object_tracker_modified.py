...
def main(_argv):
    ...
    #Mouse Monitering (whole window): size 800 * 600
    #real video place: left-top point = (10, 110), bottom-left point = (810, 710) 
    #clicking box: left-top point = (1120, 430), bottom-left point = (1920, 1030)
    def on_click(x, y, _, is_pressed):
        if is_pressed and 1120 <= x and x <= 1920 and 430 <= y and y <= 1030:
            global single, point, trackerid
            if not single:
                single = True
                x -= 1110
    ...
    process = (
        ...
        #new added
        #.setpts('1.7*PTS')
        .output('../try.m3u8', format = 'hls', pix_fmt = 'yuv420p', vcodec = 'libx264', preset = "ultrafast", hls_time = 10, hls_list_size = 2, start_number = 0, hls_flags = 'delete_segments+append_list', hls_segment_filename = '../try_%03d.ts')
        .overwrite_output()
    ...
    process.stdin.close()
    process.wait()
    #p = sp.call('ffmpeg.exe -f vfwcap -i ' + '-' + ' -r ' + strfps + ' -y -s ' + '{}x{}'.format(width, height) + ' -c:v libx264 -hls_time 10 -hls_list_size 10 -start_number 1 -hls_flags delete_segments try.m3u8', shell = True)
